...
const SYSTEM_PROMPT = `你是大麻成癮（Cannabis Use Disorder, CUD）領域的資深研究員與科學傳播者。你的任務是：
...
const ALLOWED_TAGS = [
  '大麻使用障礙', '大麻戒斷', '大麻渴求', '大麻成癮治療',
  '認知行為治療', '動機式訪談', '條件管理', '藥物治療',
  '神經生物學', '內源性大麻素', '精神共病',
  '憂鬱症', '焦慮症', '精神病', 'PTSD', 'ADHD',
  '青少年大麻使用', '大麻政策', '大麻合法化',
  '公共衛生', '流行病學', '預防科學',
  '臨床試驗', '系統性回顧', '社會決定因素',
  '大麻神經影像', 'CBD治療', '成癮機制',
...
function parseArgs() {
  ...
  const opts = { input: 'papers.json', date: '' };
...
function loadSummarizedPmids() {
  const p = resolve('docs/summarized_pmids.json');
  if (!existsSync(p)) return new Set();
  try {
    return new Set(JSON.parse(readFileSync(p, 'utf-8')).pmids || []);
  } catch { return new Set(); }
}

function saveSummarizedPmids(existing, newPmids) {
  const merged = [...new Set([...existing, ...newPmids])].sort();
  writeFileSync(resolve('docs/summarized_pmids.json'), JSON.stringify({ pmids: merged }, null, 2), 'utf-8');
}
...
function robustJsonParse(text) {
  ...
  if (braceStart !== -1 && braceEnd > braceStart) {
    let partial = cleaned.slice(braceStart, braceEnd + 1);
    for (let depth = 0; depth < 5; depth++) {
      try { return JSON.parse(partial); } catch (e) {
        const idx = partial.lastIndexOf(',');
        if (idx === -1) break;
        const openBraces = (partial.slice(0, idx).match(/{/g) || []).length;
        const closeBraces = (partial.slice(0, idx).match(/}/g) || []).length;
        const openBrackets = (partial.slice(0, idx).match(/\[/g) || []).length;
        const closeBrackets = (partial.slice(0, idx).match(/]/g) || []).length;
        partial = partial.slice(0, idx) + ']'.repeat(openBrackets - closeBrackets) + '}'.repeat(openBraces - closeBraces);
      }
    }
  }
  return null;
...
async function callZhipuAPI(apiKey, papersData) {
  ...
  const safePapers = (papersData.papers || []).map(p => ({
    pmid: sanitizeForPrompt(String(p.pmid || '')),
    title: sanitizeForPrompt(p.title || ''),
    journal: sanitizeForPrompt(p.journal || ''),
    abstract: sanitizeForPrompt(p.abstract || ''),
    url: p.url || '',
    keywords: (p.keywords || []).slice(0, 10).map(k => sanitizeForPrompt(k)),
  }));
  const papersText = JSON.stringify(safePapers, null, 2);

  const userPrompt = `以下是 ${dateStr} 從 PubMed 抓取的最新大麻成癮（Cannabis Use Disorder）相關文獻（共 ${count} 篇新論文）。
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        ...
        if (!resp.ok) {
          ...
          console.error(`[ERROR] ${model} HTTP ${resp.status}: ${errText.slice(0, 200)}`);
        ...
        console.error(`[INFO] Analysis complete: ${parsed.top_picks?.length || 0} top picks, ${parsed.all_papers?.length || 0} total`);
        return parsed;
      } catch (e) {
...
<title>Cannabis Research Daily &middot; 大麻成癮研究文獻日報 &middot; ${dateDisplay}</title>
<meta name="description" content="${dateDisplay} 大麻成癮（Cannabis Use Disorder）研究文獻日報，由 AI 自動彙整 PubMed 最新論文"/>
<style>
...
<div class="container">
  <header>
    ...
    <div class="header-text">
      <h1>Cannabis Research Daily &middot; 大麻成癮研究文獻日報</h1>
...
async function main() {
  ...
  const dateStr = opts.date || papersData.date || new Date().toISOString().slice(0, 10);

  if (!papersData.papers || papersData.papers.length === 0) {
    console.error('[WARN] No new papers to summarize');
    const emptyAnalysis = {
      date: dateStr,
      market_summary: '今日 PubMed 暫無新的大麻成癮（CUD）相關文獻更新。請明天再查看。',
      top_picks: [],
      all_papers: [],
      keywords: [],
      topic_distribution: {},
    };
    const html = generateHtml(emptyAnalysis, dateStr);
    mkdirSync('docs', { recursive: true });
    writeFileSync(resolve(`docs/cannabis-${dateStr}.html`), html, 'utf-8');
    console.error(`[INFO] Empty report saved: docs/cannabis-${dateStr}.html`);
    return;
  }
  ...
  mkdirSync('docs', { recursive: true });
  const outputPath = resolve(`docs/cannabis-${dateStr}.html`);
  writeFileSync(outputPath, html, 'utf-8');
  console.error(`[INFO] Report saved: ${outputPath}`);

  const newPmids = papersData.papers.map(p => p.pmid).filter(Boolean);
  const existing = loadSummarizedPmids();
  saveSummarizedPmids(existing, newPmids);
  console.error(`[INFO] Updated summarized PMIDs: ${existing.size} → ${existing.size + newPmids.length}`);
}
